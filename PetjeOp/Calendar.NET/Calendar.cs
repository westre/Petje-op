        ...
        private void CalendarMouseMove(object sender, MouseEventArgs e)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                    ...
                    (_calendarView == CalendarViews.Day && z.EventArea.Contains(e.X, e.Y + _scrollPanel.ScrollOffset) && z.Event.TooltipEnabled))
                {
                    ...
                    if(z.Event is CustomEvent) {
                        CustomEvent customEvent = (CustomEvent)z.Event;

                        if(customEvent.CustomToolTipText != "") {
                            _eventTip.EventToolTipText = customEvent.CustomToolTipText;
                        }
                        else {
